...
Deno.serve(async (req) => {
  ...
  try {
    // 1) Input: orderId y paymentProof obligatorios.
    const { orderId, paymentProof } = await req.json().catch(() => ({}));
    if (!orderId || !paymentProof) return json({ error: 'INVALID_INPUT' }, 400);
    ...
    // 4) Validar paymentProof. Inválido → Order queda PENDING (no materializa); el
    //    TTL (expire_orders) la reclama. confirm_order nunca escribe un terminal aquí.
