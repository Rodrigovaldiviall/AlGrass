-- When a guest cancels (game_players.status: confirmed → canceled),
-- credit the unit_price back to the titular payer via wallet_transactions.
...
CREATE TRIGGER on_guest_canceled
  AFTER UPDATE ON game_players
  FOR EACH ROW EXECUTE FUNCTION handle_guest_cancellation();
