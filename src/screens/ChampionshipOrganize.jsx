...
export default function ChampionshipOrganize() {
  ...
  // Esta pantalla es SOLO el formulario. La intro vive en su propia ruta (/championships/intro y /empresas).
  // introSource = de qué intro se entró → destino del "atrás" del form. Persiste en el draft para sobrevivir
  // a reload/round-trip a /venue. View-return (restore de "Ver mi campeonato") NO trae introSource → back a la lista.
  const introSource = location.state?.introSource ?? restore?.introSource ?? null;
  ...
  // Un ref a la última versión evita capturar un cierre obsoleto. Esta pantalla es SIEMPRE el formulario
  // (la intro vive en su propia ruta), así que se persiste sin condición de paso.
  const buildRef = useRef(buildOrganizeState); buildRef.current = buildOrganizeState;
  useEffect(() => () => { try { sessionStorage.setItem(ORG_DRAFT_KEY, JSON.stringify(buildRef.current())); } catch {} }, []);
  ...
  // Abre el detalle del VENUE reutilizando /venue (VenueDetail). En Campeonatos la foto/ubicación son
  // del VENUE (no de una cancha concreta): se pasa SOLO venueCover* (sin fieldCoverPath) → VenueDetail
  // resuelve venue cover → placeholder. Volver = navigate(-1) (Organize se restaura vía draft existente).
  function openVenueDetail() {
    if (!resolvedVenue) return;
    const chips = Object.entries(resolvedVenue.amenities || {})
      .filter(([k, val]) => val === true && AMENITY_LABEL[k])
      .map(([k]) => ({ kind: k, label: AMENITY_LABEL[k] }));
    const courts = resolvedVenue.courts;
    navigate('/venue', {
      state: {
        venue: {
          venueName: resolvedVenue.name,
          name: courts ? `${courts} ${courts === 1 ? 'cancha disponible' : 'canchas disponibles'}` : resolvedVenue.name,
          address: resolvedVenue.address,
          district: resolvedVenue.district,
          city: resolvedVenue.city,
          lat: resolvedVenue.lat,
          lng: resolvedVenue.lng,
          // FOTO DEL VENUE (compartida por todas sus canchas): solo venueCover*, SIN fieldCoverPath.
          venueCoverPath: resolvedVenue.coverPath,
          venueCoverVersion: resolvedVenue.coverVersion,
          chips,
        },
      },
    });
            ...
            {invLoading ? (
              ...
              <div style={{ minHeight: 270, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '8px 12px' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
                  ...
                  {/* Bloque CENTRAL clicable → detalle del venue (/venue). Las flechas (aparte) siguen en cycleVenue. */}
                  <div role="button" onClick={openVenueDetail} style={{ flex: 1, textAlign: 'center', minWidth: 0, cursor: 'pointer', WebkitTapHighlightColor: 'transparent' }}>
